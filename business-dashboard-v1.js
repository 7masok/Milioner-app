...
function businessEnsureStyle(){
 ...
 .business-hour{position:relative;min-width:0}.business-yesterday-bar,.business-bar{position:absolute;left:24%;width:52%;min-height:1px}.business-yesterday-bar{border-radius:5px 5px 1px 1px;background:#d1d1d6;border-top:3px solid #9d9da3;box-sizing:border-box;z-index:1}.business-yesterday-bar.negative{border-radius:1px 1px 5px 5px;border-top:0;border-bottom:3px solid #9d9da3}.business-bar{border-radius:5px 5px 1px 1px;background:#111;z-index:2}.business-bar.negative{border-radius:1px 1px 5px 5px;background:#8d2222}
...
function businessPaintChart(model,field){
 ...
 const bars=model.buckets.map((b,i)=>{
  const v=values[i],yv=Number(yesterdayValues[i])||0,
    ...
    sameDirection=(v===0||yv===0||Math.sign(v)===Math.sign(yv)),
    equal=sameDirection&&Math.abs(v-yv)<0.005,
    todayShorter=sameDirection&&!equal&&Math.abs(v)<Math.abs(yv),
    yesterdayShorter=sameDirection&&!equal&&Math.abs(yv)<Math.abs(v),
    yZ=yesterdayShorter?3:1,tZ=todayShorter||equal?3:2,
    todayBottom=equal&&v<0?`calc(${bottom}% + 3px)`:`${bottom}%`,
    todayHeight=equal?`calc(${Math.max(0,height)}% - 3px)`:`${Math.max(v===0?0:1.2,height)}%`;
  return `<div class="business-hour" title="${b.label}:00 · сегодня ${businessMoney(v)} · вчера ${businessMoney(yv)}"><div class="business-yesterday-bar ${yv<0?'negative':''}" style="bottom:${yBottom}%;height:${Math.max(yv===0?0:1.2,yHeight)}%;z-index:${yZ}"></div><div class="business-bar ${v<0?'negative':''}" style="bottom:${todayBottom};height:${todayHeight};z-index:${tZ}"></div></div>`;
 }).join('');
