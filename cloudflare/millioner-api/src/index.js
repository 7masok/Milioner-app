...
export default {
  async fetch(request, env, ctx) {
    ...
    // WAREHOUSE_FAST_READ_V4: hot warehouse reads must not run the full schema chain.
    ...
    if (url.pathname === '/api/kaspi-report-orders' && request.method === 'GET') {
      ...
      try {
        ...
        else recoveryQueued = queueKaspiReportLineRecovery(ctx, env, orders, 36);
    ...
    try {
      ...
      if (url.pathname === '/api/wb-sales-live' && request.method === 'GET') {
        const market=normalizeMarket(url.searchParams.get('market'));
        if(!['WB','WB2'].includes(market)) return json({ok:false,error:'market must be WB or WB2'},400,cors);
        const token=String((market==='WB2'?env.WB_TOKEN_2:env.WB_TOKEN)||'').trim();
...
function kaspiReportPeriodBounds(days=1, rawFrom=0, rawTo=0){
  const offset=5*60*60*1000;
...
async function fetchKaspiOrdersDirect(token, { days = 7, state = 'KASPI_DELIVERY', status = '' } = {}) {
  const headers = {
