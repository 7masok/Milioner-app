...
try{
  ...
  function ozonProfitModel(payload,days){
    const bounds=ozonBounds(days),maps=buildOzonMaps(payload),groups=new Map(),products=new Map();let unallocated=0;
    ...
    for(const account of payload?.accounts||[])for(const raw of account?.finance?.rows||[]){
      ...
      if(!product){if(category!=='sales')unallocated+=amount;continue;}
