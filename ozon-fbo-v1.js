...
function resolveProductIdForOzonLine(offer_id,sku){
 if(!offer_id&&!sku)return null;
 const offer=String(offer_id||'').trim(),skuStr=String(sku||'').trim(),keys=[offer,skuStr].filter(Boolean);
 for(const p of state.products||[]){const pKeys=productOzonKeys(p);for(const k of keys){if(k&&pKeys.includes(k))return p.id;}}
 return null;
}
function populateOzonOrderFeed(){
 const out=[];
 for(const account of data?.accounts||[]){
  const postings=account.postings?.rows||[];
  for(const posting of postings){
   const createdAt=Date.parse(posting.created_at||posting.in_process_at||'')||0;
   const products=posting.products||[];
   if(!products.length)continue;
   for(const product of products){
    const offer_id=String(product.offer_id||'').trim(),sku=String(product.sku||'').trim(),offer=offer_id||sku,qty=Number(product.quantity)||0,price=Number(product.price)||0,totalPrice=price*qty;
    if(!offer||!qty)continue;
    const productId=resolveProductIdForOzonLine(offer_id,sku)||null;
    out.push({
     market:'Ozon',
     orderId:String(posting.posting_number||'').trim(),
     code:String(posting.posting_number||'').trim(),
     entryId:String(offer_id)+'_'+String(product.sku||'unknown'),
     sku:offer_id||sku,
     ...
     state:String(posting.status||'').trim(),
     productId:productId
    });
   }
  }
 }
 state.ozonOrderFeed=out;
}
function draw(){
 ...
 if(selectedOrderMarket==='Ozon'){
  ...
  for(const a of accounts)for(const p of a.postings?.rows||[]){
   count++;
   if(p.status==='cancelled')continue;
   for(const x of p.products||[]){
    qty+=Number(x.quantity||0);
    const c=String(x.currency_code||p.financial_data?.currency_code||'').toUpperCase()||'—';
    amounts[c]=(amounts[c]||0)+Number(x.price||0)*Number(x.quantity||0);
   }
  }
  ...
  (!data?'<div class=\"empty\">Загружаю Ozon…</div>':!data.configured?'<div class=\"empty\">Добавьте API-ключ Ozon в настройках.</div>':!accounts.length?'<div class=\"empty\">Первая загрузка Ozon выполняется…</div>':'');
 }
}
async function load(force=false){
 if(loading)return data;if(!force&&data&&Date.now()-loadedAt<60000)return data;
 loading=true;
 try{data=await apiJson(MILLIONER_API+'/api/ozon-fbo');loadedAt=Date.now();message='';rebuildFboMap();populateOzonOrderFeed();updateFboMetric();}
