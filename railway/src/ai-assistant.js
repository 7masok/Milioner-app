...
export function parseBccStatement(text, sourceHash, filename) {
  ...
  const tableText = posted.replace(
    /^(\s*\d{4}-\d{2}-\d{2}\s+\d{4}-\d{2}-\d{2}\s+.+?\s+[\d ]+\.\d{2})\s+([+-]?[\d ]+\.\d)([^\n]*)\n\s*([A-Z]{3})\s+0\s+([A-Z]{3})([^\n]*)$/gmi,
    (_all,prefix,accountAmount,lineTail,originalCurrency,accountCurrency,nextTail)=>
      prefix+' '+originalCurrency+' '+accountAmount+'0 '+accountCurrency+' '+String(lineTail||'').trim()+' '+String(nextTail||'').trim()
  );
...
aiAssistantRouter.post('/assistant/chat', requireTrustedOrigin, asyncRoute(async (req,res)=>{
  ...
  const context=snapshotSummary(await hydrateWarehouseSales(pool, await hydrateWarehousePurchases(pool, parseWarehousePayload(result.rows[0]?.payload))),ordersResult.rows);
