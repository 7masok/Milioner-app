...
async function readFinanceState(client) {
  const [accounts, categories, transactions, imports, meta] = await Promise.all([
    client.query('SELECT payload FROM finance_accounts ORDER BY sort_order,id'),
    client.query('SELECT payload FROM finance_categories ORDER BY sort_order,id'),
    client.query('SELECT payload FROM finance_transactions ORDER BY sort_order,id'),
