...
async function readFinanceState(client) {
  const [accounts, categories, transactions, imports, meta] = await Promise.all([
    client.query("SELECT payload || jsonb_build_object('id',id) AS payload FROM finance_accounts ORDER BY sort_order,id"),
    client.query("SELECT payload || jsonb_build_object('id',id,'name',name,'kind',kind) AS payload FROM finance_categories ORDER BY sort_order,id"),
    client.query("SELECT payload || jsonb_build_object('id',id) AS payload FROM finance_transactions ORDER BY sort_order,id"),
