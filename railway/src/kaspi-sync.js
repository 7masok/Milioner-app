...
export async function syncKaspiOrders({ days = 2 } = {}) {
  ...
  syncInFlight = (async () => {
    ...
    try {
      ...
      if (compositionBackfill.checked || compositionBackfill.recovered || compositionBackfill.failed) console.info('Kaspi composition backfill:', JSON.stringify(compositionBackfill));
