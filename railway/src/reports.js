...
reportsRouter.get('/wb-finance-summary', asyncRoute(async (req, res) => {
  ...
  res.json({ ok: true, market: selected, days, range: { since, until, timezone: 'Asia/Almaty' }, sync: sync.rows[0] || null,
    lastSuccess: lastSuccess.rows[0] || null, financeAvailable: Number(row.financeRowCount || 0) > 0, ...row,
