...
async function warehouseKaspiRows(){
  ...
  for(const recovery of KASPI_RECOVERY_OFFERS){if(combined.has(recovery.sku))continue;const product=recoveryProduct(Array.isArray(snapshot.products)?snapshot.products:[],recovery.model);if(!product)continue;const productId=String(product.id||'');combined.set(recovery.sku,{sku:recovery.sku,productId,name:String(product.name||recovery.model),stock:availability.available(productId)})}
...
async function liveKaspiXml() {
  ...
  const recovery=KASPI_RECOVERY_OFFERS.filter(x=>stocks.has(x.sku)&&!info.offers.has(x.sku)).map(x=>({...x,stock:stocks.get(x.sku),storeId:primaryStoreId}));
  raw=appendOffers(raw,recovery);info=templateInfo(raw);
  const offerRe=/<offer\b[^>]*\bsku\s*=\s*(["'])([^"']+)\1[^>]*>[\s\S]*?<\/offer>/gi;
