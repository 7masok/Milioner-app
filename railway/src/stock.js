...
function xmlAttr(tag, name) { const match = new RegExp(`\\b${name}\\s*=\\s*(["'])([^"']*)\\1`, 'i').exec(String(tag || '')); return match ? xmlDecode(match[2]) : ''; }
function setXmlAttr(tag, name, value) { const safe = xmlEscape(value); const re = new RegExp(`(\\s${name}\\s*=\\s*)(["'])([^"']*)\\2`, 'i'); if (re.test(tag)) return tag.replace(re, (_m, prefix) => `${prefix}"${safe}"`); return tag.replace(/\s*\/?>$/, tail => ` ${name}="${safe}"${tail.trim().startsWith('/') ? '/>' : '>'}`); }
function makeAvailability(storeId, available, stockCount) { return `<availability storeId="${xmlEscape(storeId)}" available="${available ? 'yes' : 'no'}" stockCount="${Math.max(0, Math.floor(stockCount))}"/>`; }
...
function templateInfo(xml) {
  ...
  while ((offer = offerRe.exec(raw))) {
    const sku = xmlDecode(offer[2]).trim(); if (!sku) continue; offerSkus.add(sku);
    const currentStores = new Set(); const availabilityRe = /<availability\b[^>]*\bstoreId\s*=\s*(["'])([^"']+)\1[^>]*\/?>/gi; let availability;
    while ((availability = availabilityRe.exec(offer[0]))) { const store = xmlDecode(availability[2]).trim(); if (store) { stores.add(store); currentStores.add(store); } }
...
async function status(req) {
  const [row, access, state] = await Promise.all([templateRow(), pool.query('SELECT last_fetched_at AS "lastFetchedAt",fetch_count AS "fetchCount",last_user_agent AS "lastFetchUserAgent" FROM kaspi_price_feed_access WHERE id=1'), warehouse()]);
...
stockRouter.put('/kaspi-price-template', requireTrustedOrigin, requireWritesEnabled, asyncRoute(async (req, res) => {
  const existing = await templateRow(); const raw = typeof req.body?.xml === 'string' && req.body.xml ? req.body.xml : existing?.rawXml; if (!raw) throw httpError('Сначала загрузите текущий XML-прайс Kaspi.'); if (Buffer.byteLength(raw, 'utf8') > 6_000_000) throw httpError('XML-прайс слишком большой для этого импорта (лимит 6 МБ).', 413); const info = templateInfo(raw); let primaryStoreId = String(req.body?.primaryStoreId || existing?.primaryStoreId || '').trim(); if (primaryStoreId && !info.storeIds.includes(primaryStoreId)) throw httpError('Выбранного склада нет в загруженном XML Kaspi.'); if (!primaryStoreId && info.storeIds.length === 1) primaryStoreId = info.storeIds[0]; const feedKey = String(existing?.feedKey || '') || crypto.randomBytes(24).toString('hex');
  await pool.query(`INSERT INTO kaspi_price_template(id,raw_xml,feed_key,primary_store_id,offer_count,store_ids,merchant_id,updated_at) VALUES(1,$1,$2,$3,$4,$5,$6,$7) ON CONFLICT(id) DO UPDATE SET raw_xml=excluded.raw_xml,feed_key=excluded.feed_key,primary_store_id=excluded.primary_store_id,offer_count=excluded.offer_count,store_ids=excluded.store_ids,merchant_id=excluded.merchant_id,updated_at=excluded.updated_at`, [raw, feedKey, primaryStoreId, info.offerSkus.size, JSON.stringify(info.storeIds), info.merchantId, Date.now()]); res.json({ ok: true, ...(await status(req)) });
}));
stockRouter.get('/kaspi-price-template', asyncRoute(async (_req, res) => { const row = await templateRow(); res.json({ ok: true, configured: Boolean(row?.rawXml), ...row, rawXml: undefined }); }));
stockRouter.get('/stock-sync-status', asyncRoute(async (_req, res) => { const rows = []; for (const selected of ['WB', 'WB2']) { const latest = await pool.query('SELECT * FROM stock_sync_runs WHERE market=$1 ORDER BY id DESC LIMIT 1', [selected]); rows.push({ market: selected, latest: latest.rows[0] || null }); } res.json({ ok: true, markets: rows }); }));
stockRouter.post('/stock-sync-now', requireTrustedOrigin, requireWritesEnabled, asyncRoute(async (_req, res) => { if (!config.marketSyncEnabled) return res.status(503).json({ ok: false, error: 'market-sync-disabled-during-migration' }); return res.status(501).json({ ok: false, error: 'wb-stock-sync-port-not-enabled' }); }));

export const kaspiFeedHandler = asyncRoute(async (req, res) => {
  const row = await templateRow(); if (!row?.rawXml || !row?.feedKey || String(req.query.key || '') !== String(row.feedKey)) throw httpError('Not found', 404); const state = await warehouse(); if (!(state.products || []).length) throw httpError('Warehouse is empty; Kaspi feed blocked by safety gate.', 503); const info = templateInfo(row.rawXml); const primaryStoreId = String(row.primaryStoreId || '') || (info.storeIds.length === 1 ? info.storeIds[0] : ''); if (!primaryStoreId) throw httpError('Primary Kaspi store is not selected.', 409);
  const managed = new Map((state.products || []).map(product => [String(product?.kaspi || '').trim(), product]).filter(([sku]) => sku)); const reserved = new Map();
  for (const item of state.reservations || []) if (item?.active && ['WB','WB2'].includes(String(item.source || ''))) reserved.set(String(item.productId), (reserved.get(String(item.productId)) || 0) + Math.max(0, Number(item.qty) || 0));
  let matched = 0;
  ...
  const xml = row.rawXml.replace(offerRe, (whole, open, _quote, encodedSku, body, close) => {
    const sku = xmlDecode(encodedSku).trim(); const product = managed.get(sku); if (!product) return whole; matched++;
  ...
  await pool.query(`INSERT INTO kaspi_price_feed_access(id,last_fetched_at,fetch_count,last_user_agent) VALUES(1,$1,1,$2) ON CONFLICT(id) DO UPDATE SET last_fetched_at=excluded.last_fetched_at,fetch_count=kaspi_price_feed_access.fetch_count+1,last_user_agent=excluded.last_user_agent`, [Date.now(), String(req.headers['user-agent'] || '').slice(0, 300)]); res.type('application/xml').send(xml);
});
