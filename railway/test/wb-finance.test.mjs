...
test('WB live sales cache is refreshed from the operational sales and returns API', () => {
  const source = readFileSync(new URL('../src/wb-sync.js', import.meta.url), 'utf8');
  assert.match(source, /statistics-api\.wildberries\.ru/);
  assert.match(source, /\/api\/v1\/supplier\/sales/);
  assert.match(source, /LIVE_SALES_SYNC_MS = 31 \* 60 \* 1000/);
  ...
  assert.match(source, /wbMoscowDateTime/);
  assert.match(source, /raw \+ '\+03:00'/);
  assert.match(source, /const liveSales = await syncLiveSales\(market, token, \{ force \}\)/);
  assert.doesNotMatch(source, /liveSalesDisabled:\s*true/);
  assert.match(source, /export async function syncWbLiveSales/);
