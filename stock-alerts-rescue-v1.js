...
function stockAlertFor(p){
  ...
  const target=Math.ceil(daily*COVER_DAYS),buyQty=Math.max(0,target-available-warehouse-coming),signature=[book,reserve,warehouse,coming].join(':');
...
function currentAlerts(){const hidden=readHidden(),alerts=allAlerts(),activeIds=new Set(alerts.map(x=>String(x.p.id)));let changed=false,visible=[];for(const key of Object.keys(hidden)){if(!activeIds.has(String(key))){delete hidden[key];changed=true}}for(const x of alerts){const key=String(x.p.id),saved=hidden[key];if(saved&&saved===x.signature)continue;if(saved&&saved!==x.signature){delete hidden[key];changed=true}visible.push(x)}if(changed)writeHidden(hidden);return visible}
...
function alertCard(x){const pid=String(x.p.id).replace(/'/g,"\\'");const action=x.warehouse>0?`На складе ждёт ввода в продажу: <b>${x.warehouse} шт.</b>`:x.buyQty>0?`Рекомендуется закупить примерно <b>${x.buyQty} шт.</b>`:'Поставка уже в пути';return `<div class="item stock-alert-card" onclick="openStockAlertProduct('${pid}')"><button type="button" class="stock-alert-dismiss" aria-label="Скрыть" title="Скрыть · я в курсе" onclick="event.stopPropagation();hideStockAlert('${pid}')">×</button><div class="row"><div class="grow"><div class="name">${escText(x.p.name)}</div><div class="muted">Запаса к продаже осталось примерно на ${dayText(x.days)}</div></div><span class="badge bad">${x.available} шт.</span></div><div class="stock-alert-grid"><div class="stock-alert-mini"><div class="label">По учёту в продаже</div><b>${x.book} шт.</b></div><div class="stock-alert-mini"><div class="label">В резерве</div><b>${x.reserve} шт.</b></div><div class="stock-alert-mini"><div class="label">Темп продаж</div><b>${x.daily.toFixed(1)} шт./день</b></div><div class="stock-alert-mini"><div class="label">В пути</div><b>${x.coming} шт.</b></div></div><div class="muted" style="margin-top:9px">${action}. Нажмите, чтобы пересчитать товар и начать инвентаризацию.</div><div class="actions" onclick="event.stopPropagation()">${x.warehouse>0?`<button class="btn" onclick="openStockAlertWarehouse('${pid}')">Со склада</button>`:''}<button class="btn" onclick="openStockAlertPurchase('${pid}')">Закупить</button><button class="btn dark" onclick="openStockAlertProduct('${pid}')">Пересчитать</button></div></div>`}
window.openStockAlerts=function(){const alerts=currentAlerts();if(!alerts.length)return alert('Сейчас нет новых уведомлений по остаткам.');showSheet(`<h3>Нужно проверить остаток · ${alerts.length}</h3><div class="link-note">Крестик × означает «я в курсе». Уведомление вернётся, если изменится остаток, резерв, товар на складе или в пути.</div>${alerts.map(alertCard).join('')}`)};
window.hideStockAlert=function(pid){const x=stockAlertFor(findProduct(pid));if(!x)return;const hidden=readHidden();hidden[String(pid)]=x.signature;writeHidden(hidden);paintBell();const left=currentAlerts();if(!left.length){closeModal();return}window.openStockAlerts()};
